    ...
    private:
        auto page_display_options() {
            ...
            return fieldset{{_class{"fieldset p-4 rounded-box shadow h-full flex flex-col"}},
                legend{{_class{"fieldset-legend"}}, "Display Options"_},
                ctx.on_input(textarea{{_id{"stencil_textarea"}, _class{"textarea w-full min-h-[2.5rem]"}, _rows{"1"}, _placeholder{"Log line stencil. Use {{...}} to insert values."_}},},
                    [this](webpp::event) {
                        ...
                        dv{{_class{"flex flex-row md:flex-col gap-4"}},
                            ...
                            dv{{_class{"dropdown dropdown-hover dropdown-top md:dropdown-bottom dropdown-end"}},
                                ...
                                ul{{_class{"dropdown-content menu bg-base-300 rounded-box z-1 w-full p-1 shadow-sm"}},
                                    each(std::array{
                                        std::make_pair<std::string_view, unsigned int>("100 entries"_, 100),
                                        std::make_pair<std::string_view, unsigned int>("1k entries"_, 1000),
                                        std::make_pair<std::string_view, unsigned int>("10k entries"_, 10000),
                                        std::make_pair<std::string_view, unsigned int>("100k entries"_, 100000),
                                        std::make_pair<std::string_view, unsigned int>("1M entries"_, 1000000),
                                    }, [&](const auto& e) {
